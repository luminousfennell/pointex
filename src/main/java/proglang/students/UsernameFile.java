...
public class UsernameFile {
	...
	/**	
	 * Throws an IllegalArgumentException if the line is likely not a username
	 * @param line The line to test
	 */
	static void checkValidity(String line) {

		boolean valid = line.length() >= 3 
				&& (Character.isAlphabetic(line.toCharArray()[0])) ;
				// Stricter checks (see below) create problems and are likely unecessary...
//						&& Character.isAlphabetic(line.toCharArray()[1]) && allNumeric(line.substring(2))
